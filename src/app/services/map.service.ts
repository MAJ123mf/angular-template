...
export class MapService {
  ...
  constructor(
    ...
    private languageService: LanguageService   // dodano za prevode slojev
  ) { 
      ...
      // Posodobi title-je slojev ob menjavi jezika
  ...
  // Posodobi title-je vseh slojev ob menjavi jezika
  private updateLayerTitles(): void {
    // Delovni vektorski sloji
    this.parcelsVectorLayer?.set('title', this.languageService.instant('LAYER_PARCELS_WORKING'));
    this.buildingsVectorLayer?.set('title', this.languageService.instant('LAYER_BUILDINGS_WORKING'));
    this.roadsVectorLayer?.set('title', this.languageService.instant('LAYER_ROADS_WORKING'));
    this.addressVectorLayer?.set('title', this.languageService.instant('LAYER_ADDRESSES_WORKING'));
 
    // Skupine
    this.myWorkingLayersGroup?.set('title', this.languageService.instant('LAYERGROUP_MY_WORKING'));
    this.myLayersGroup?.set('title', this.languageService.instant('LAYERGROUP_MY_WMS'));
    this.baseLayersGroup?.set('title', this.languageService.instant('LAYERGROUP_WMS_GURS'));
    this.WFSLayersGroup?.set('title', this.languageService.instant('LAYERGROUP_WFS_GURS'));
 
    // WMS GURS sloji (dostopamo jih prek baseLayersGroup)
    const layersList = this.baseLayersGroup?.getLayers().getArray();
    if (layersList && layersList.length >= 3) {
      layersList[0]?.set('title', this.languageService.instant('LAYER_WMS_PARCELS'));
      layersList[1]?.set('title', this.languageService.instant('LAYER_WMS_BUILDINGS'));
      layersList[2]?.set('title', this.languageService.instant('LAYER_WMS_DOF'));
    }
 
    // WFS GURS sloji
    const wfsLayers = this.WFSLayersGroup?.getLayers().getArray();
    if (wfsLayers && wfsLayers.length >= 4) {
      wfsLayers[0]?.set('title', this.languageService.instant('LAYER_WFS_KO'));
      wfsLayers[1]?.set('title', this.languageService.instant('LAYER_WFS_PARCELS'));
      wfsLayers[2]?.set('title', this.languageService.instant('LAYER_WFS_BUILDINGS'));
      wfsLayers[3]?.set('title', this.languageService.instant('LAYER_WFS_MUNICIPALITIES'));
    }

    // Lokalni WFS sloji
    this.localWFSLayersGroup?.set('title', this.languageService.instant('LAYERGROUP_LOCAL_WFS'));
    const localWfsLayers = this.localWFSLayersGroup?.getLayers().getArray();
    if (localWfsLayers && localWfsLayers.length >= 4) {
      localWfsLayers[0]?.set('title', this.languageService.instant('LAYER_LOCAL_WFS_PARCELS'));
    ...
    // Moji WMS sloji
    const myWmsLayers = this.myLayersGroup?.getLayers().getArray();
    if (myWmsLayers && myWmsLayers.length >= 4) {
      myWmsLayers[0]?.set('title', this.languageService.instant('LAYER_PARCELS_WMS'));
  ...
  private clearInteractions(): void {
    ...
    if (this.modifyParcelInteraction) {
      this.map.removeInteraction(this.modifyParcelInteraction);
      this.modifyParcelInteraction = undefined;
    }
    if (this.modifyBuildingInteraction) {
      this.map.removeInteraction(this.modifyBuildingInteraction);
      this.modifyBuildingInteraction = undefined;
    }
    if (this.modifyInteraction) {          // roads
      this.map.removeInteraction(this.modifyInteraction);
      this.modifyInteraction = undefined;
    }
    if (this.modifyAddressInteraction) {   // addresses
      this.map.removeInteraction(this.modifyAddressInteraction);
      this.modifyAddressInteraction = undefined;
    }
  ...
  createSentinelLayers(): LayerGroup {
   ...
   // WFS layer: NEP_KATASTRSKE_OBCINE
    ...
    // Lokalni WFS sloji - to so sloji, ki jih imamo v svojem Geoserverju, ki črpa podatke iz naše Postgres baze. V tej funkciji dodamo 4 sloje: originalne parcele in poenostavitve parcel na 0.5m, 1m in 2m. Vsi ti sloji so WFS, torej vektorski in ne rastrski kot WMS. To pomeni, da se geometrije prenašajo na frontend in se tam rišejo, medtem ko WMS prenaša že narisano sliko s strežnika.
    createLocalWFSLayers(): LayerGroup {
  ...
  createMyLayers(): LayerGroup {
   ...
   createMyWorkingLayers(): LayerGroup {
     ...
     this.parcelsVectorLayer = new VectorLayer({
       ...
       properties: {
         title: 'Parcels vector' // 
         // displayName: 'Parcele'   // Ime, ki ga želim prikazati v meniju    
  ...
  createMap(): Map { 
    ...
    this.map = new Map({
      ...
      view: new View({
        ...
        maxResolution: 30,   // ← zoom out omejitev
      }),
      layers: [
 ...
 */
  ...
  private activateEditParcel(): void {
    ...
    this.snapInteraction = new Snap({ source: vectorSource });

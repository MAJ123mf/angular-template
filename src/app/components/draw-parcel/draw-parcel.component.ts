...
export class DrawParcelComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.eventService.eventActivated$.subscribe((event: EventModel) => {
      if (event.type === 'parcelsAddedToMap') {
        ...
        const sourceParcels = this.mapService.parcelsLayer?.getSource();
        const layer = this.mapService.parcelsLayer;
  ...
  addSnapCursorFeedback() {
    ...
    const source = this.mapService.parcelsLayer?.getSource();
  ...
  toggleDrawMode() {
    ...
    if (this.drawMode) {
      ...
      this.mapService.parcelsLayer?.setVisible(true);  // za layer switcher  Avtomatsko vklopi vidnost sloja
  ...
  editParcel(): void {
    ...
    if (this.editMode) {
      this.mapService.parcelsLayer?.setVisible(true);  // <--- ob začetku urejanja
    }
  ...
  clearVectorLayer(){
    ...
    this.mapService.parcelsLayer?.getSource()?.clear();
  }

...
export class DrawBuildingComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.eventService.eventActivated$.subscribe((event: EventModel) => {
      if (event.type === 'buildingsAddedToMap') {
        ...
        const sourceBuildings = this.mapService.buildingsLayer?.getSource();
  ...
  addSnapCursorFeedback() {
    ...
    const source = this.mapService.buildingsVectorSource;
  ...
  toggleDrawMode() {
    ...
    if (this.drawMode) {
      ...
      this.mapService.buildingsLayer?.set('visible', true);  // za layer switcher  Avtomatsko vklopi vidnost sloja
  ...
  editBuilding(): void {
    ...
    if (this.editMode) {
      this.mapService.buildingsLayer?.setVisible(true);  // <--- ob začetku urejanja
    }
  ...
  clearVectorLayer(){
    ...
    this.mapService.buildingsLayer?.getSource()?.clear();
  }

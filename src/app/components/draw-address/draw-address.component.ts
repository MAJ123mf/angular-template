...
export class DrawAddressComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  toggleDrawMode() {
    ...
    if (this.drawMode) {
      ...
      this.mapService.addressesLayer?.set('visible', true);  // za layer switcher  Avtomatsko vklopi vidnost sloja
  ...
  addDrawAddressInteraction() {
      ...
      var sourceAddresses = this.mapService.addressVectorSource;
      if(sourceAddresses){
  ...
  clearVectorLayer(){
    ...
    this.mapService.addressesLayer?.getSource()?.clear();
  }

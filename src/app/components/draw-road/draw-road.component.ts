...
export class DrawRoadComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  toggleDrawMode(){
    ...
    if(this.drawMode){
      ...
      this.mapService.roadsLayer?.set('visible', true);  // za layer switcher  Avtomatsko vklopi vidnost sloja
  ...
  editRoad(): void {
    ...
    if (this.editMode) {
      this.mapService.roadsLayer?.set('visible', true);   // <--- ob začetku urejanja
    }
  ...
  addDrawRoadInteraction() {
    ...
    var sourceRoads = this.mapService.roadsVectorSource;
    if(sourceRoads){
  ...
  clearVectorLayer(){
    ...
    this.mapService.roadsLayer?.getSource()?.clear();
  }

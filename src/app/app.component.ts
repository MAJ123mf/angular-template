...
showHelp() {
  ...
  @HostListener('document:mousemove', ['$event'])
    onMouseMove(event: MouseEvent) {
      ...
      if (this.isResizingY) {
        // Odštej višino menuja (~3vh) in statusne vrstice (~3vh)
        const menuHeight = window.innerHeight * 0.03;
        const statusHeight = window.innerHeight * 0.03;
        const availableHeight = window.innerHeight - menuHeight - statusHeight;
        const offsetY = event.clientY - menuHeight;
        const newHeight = (offsetY / availableHeight) * 100;
  ...
  toggleTable() {
    ...
    if (this.tableCollapsed) {
      this.tableHeightBeforeCollapse = this.topHeight;
      this.topHeight = 97; // tabela skrita → karta zavzame skoraj vse
    } else {
      this.topHeight = this.tableHeightBeforeCollapse; // obnovi pred-collapse vrednost
    }
  }
